    ...
    <div className="min-h-screen w-full overflow-x-hidden">
      <div className="w-full">
        ...
        <div className="hidden lg:block">
          <div className="py-8">
            {/* Large Carousel Section - Full Width with Peek Effect */}
            <div className="relative mb-8">
              <div
                ref={desktopCarouselRef}
                className="overflow-x-scroll scrollbar-hide snap-x snap-mandatory flex items-center"
                style={{
                  WebkitOverflowScrolling: "touch",
                  scrollbarWidth: "none",
                  msOverflowStyle: "none",
                ...
                {images.map((item, index) => {
                  ...
                  return (
                    <div
                      ...
                      className={`snap-center flex-shrink-0 transition-all duration-300 cursor-pointer ${isActive ? 'scale-100 opacity-100' : 'scale-95 opacity-60'}`}
                      style={{ width: "700px" }}
                      onClick={() => scrollToDesktopSlide(index)}
                    >
                      <div className={`relative w-full ${isFloorPlan ? 'aspect-square' : 'aspect-[4/3]'} ${isFloorPlan ? 'bg-white' : 'bg-gray-100'} rounded-lg overflow-hidden shadow-lg`}>
                        {item.type === "video" ? (
                          <video
                            src={item.src}
                            controls
                          ...
                          <img
                            src={item.src || "/placeholder.svg"}
                            alt={item.alt || `${unitName} - Image ${index + 1}`}
                            className={`absolute inset-0 w-full h-full ${isFloorPlan ? 'object-contain p-4' : 'object-cover'}`}
                          />
                        )}
                      </div>
                    </div>
              ...
              <button
                ...
                disabled={current === 0}
                className="absolute left-8 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white p-3 rounded-full shadow-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all z-10"
              ...
              <button
                ...
                className="absolute right-8 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white p-3 rounded-full shadow-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all z-10"
                    ...
                    <div className="flex-shrink-0 flex gap-2">
                      {videoIndex >= 0 && (
                        <button
                          ...
                          className={`flex flex-col items-center justify-center w-16 h-16 rounded-md border-2 transition-all bg-white ${
                            current === videoIndex
                              ? "border-orange-600 ring-2 ring-orange-600 ring-opacity-50"
                              : "border-gray-300 hover:border-gray-400"
                          }`}
                        >
                          <svg className="w-5 h-5 mb-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                            <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" />
                            <polyline points="9 22 9 12 15 12 15 22" />
                          </svg>
                          <span className="text-[8px] font-medium leading-tight text-center">VIDEO<br/>TOUR</span>
                        </button>
                      )}
                      {floorPlanIndex >= 0 && (
                        <button
                        ...
                        >
                          ...
                          <span className="text-[8px] font-medium leading-tight text-center">FLOOR<br/>PLAN</span>
                        </button>
                      )}
                    </div>
                  </div>
